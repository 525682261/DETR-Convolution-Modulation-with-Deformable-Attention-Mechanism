...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries,
        ...
        if dec_pred_bbox_embed_share:
            # 如果共享同一个头，那么6层decoder使用的是同一个，否则是copy的，6个decoder都有自己的
    ...
    def init_ref_points(self, use_num_queries):
        ...
        # 参考点的中心坐标随机初始化，并固定，不会被更新
        if self.random_refpoints_xy:
            self.refpoint_embed.weight.data[:, :2].uniform_(0, 1)
            self.refpoint_embed.weight.data[:, :2] = inverse_sigmoid(self.refpoint_embed.weight.data[:, :2])
            self.refpoint_embed.weight.data[:, :2].requires_grad = False

        # 参考点的高宽随机初始化，并固定，不会被更新
    ...
    def forward(self, samples: NestedTensor, targets: List = None):
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                # 这个是bs中因不同尺寸的图像在同一个尺寸后的mask
                m = samples.mask
                # 使用插值创建新的mask
                mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]
                # backbone[1] 是位置编码，新的src和mask，并没有经过位置编码
                # 位置编码模块，这里就是调用了他们的forward方法
                pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)
                srcs.append(src)
                masks.append(mask)
                poss.append(pos_l)  # 位置编码
        # 以上的处理与Deformable DETR相同 --------------------------------------------------------------------------------

        # dn_number 100个CDN  生成噪声的代码 -----------------------------------------------------------------------------
        if self.dn_number > 0 or targets is not None:
            # dn-detr中这个方法叫prepare_for_dn
            # dn 的前处理 prepare for dn
            # 生成noise gt 以及mask
            # 这两个input开头的是去噪部分的
            # input_query_label (N,bs,256)
            # input_query_box (N,bs,4)
            # attn_mask [N,N]
            # mask_dict {known_indice, batch_idx, map_known_indice, known_lbs_bboxes, know_idx, pad_size}
        ...
        # hs 6 list [bs,N(num_queries+2*dn_number),256]
        # reference 7 list [bs,N(num_queries+2*dn_number),4]
        # enc 这两个是encoder后经过头的输出
        # hs_enc [1,bs,num_queries,256]
        # ref_enc [1,bs,num_queries,4]
        # init_box_proposal [bs,900,4] 最最初始的参考点位
        hs, reference, hs_enc, ref_enc, init_box_proposal, sampling_locations_list, attention_weights_list, memory, level_start_index, spatial_shapes, memory_list = self.transformer(srcs, masks, input_query_bbox,
                                                                             poss, input_query_label, attn_mask)
        ...
        if hs_enc is not None:
            ...
            # ref_enc 为 [1,bs,900,4] 取-1，其实也只有一个
            interm_coord = ref_enc[-1]
            # hs_enc [1,bs,900,256]
            interm_class = self.transformer.enc_out_class_embed(hs_enc[-1])
            # encoder输出的经过topk之后的，也放入到out中
            out['interm_outputs'] = {'pred_logits': interm_class, 'pred_boxes': interm_coord}
            # encoder输出的经过topk之后的，也放入到out中，init_box_proposal是在encoder时就生成的最最初始的参考点位
            out['interm_outputs_for_matching_pre'] = {'pred_logits': interm_class, 'pred_boxes': init_box_proposal}

            # prepare enc outputs
            # 还有其他的值，可能是应对多个encoder的layer的输出吧
            if hs_enc.shape[0] > 1:
                enc_outputs_coord = []
                enc_outputs_class = []
        ...
        out['dn_meta'] = dn_meta
        out['sampling_locations_list'] = sampling_locations_list
        out['attention_weights_list'] = attention_weights_list
        out['memory'] = memory
        out['level_start_index'] = level_start_index
        out['spatial_shapes'] = spatial_shapes
        out['memory_list'] = memory_list
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes = torch.full(src_logits.shape[:2], self.num_classes,
                                    dtype=torch.int64, device=src_logits.device) # 全部填充为no object
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        # indices是list, 长度是bs, 每个item是一个tuple, 第一个值是框的id，第二个值是gt的id
        ...
        # dn_meta 这个地方与DN-DETR不同了
        # prepare for dn loss {pad_size,num_dn_group,output_known_lbs_bboxes{pred_logits,pred_boxes,aux_outputs} }
        dn_meta = outputs['dn_meta']

        if self.training and dn_meta and 'output_known_lbs_bboxes' in dn_meta:
            # single_pad bs最大的gt的数量x2 (包括了正负样本的总数量)，scalar可以认为是dn-detr中的group的概念
            # output_known_lbs_bboxes的内容
            # pred_logits,pred_boxes,aux_outputs
            # pred_logits [bs,2*dn_number,91]
            # pred_boxes [bs,2*dn_number,4]
            output_known_lbs_bboxes, single_pad, scalar = self.prep_for_dn(dn_meta)
            dn_pos_idx = []
            dn_neg_idx = []
            for i in range(len(targets)):
                if len(targets[i]['labels']) > 0:
                    t = torch.range(0, len(targets[i]['labels']) - 1).long().cuda()  # 0 --> gt的数量-1
                    t = t.unsqueeze(0).repeat(scalar, 1)  # [scalar, gt num]
                    tgt_idx = t.flatten()  # (scalar*gt num)
    ...
    def prep_for_dn(self, dn_meta):
        # output_known_lbs_bboxes 是仅去噪的那部分的，不包括match的部分 pred_logits, pred_boxes, aux_outputs
        output_known_lbs_bboxes = dn_meta['output_known_lbs_bboxes']

        num_dn_groups, pad_size = dn_meta['num_dn_group'], dn_meta['pad_size']

        assert pad_size % num_dn_groups == 0
        # single_pad bs最大的gt的数量x2
