...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        # Begin Encoder
        #########################################################
        memory, enc_intermediate_output, enc_intermediate_refpoints, sampling_locations_list, attention_weights_list, memory_list = self.encoder(
            src_flatten,
            # 空间位置编码
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, sampling_locations_list, attention_weights_list, memory, level_start_index, spatial_shapes, memory_list
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not dropflag:
                ...
                if self.deformable_encoder:
                    ...
                    if layer_id == 0:
                        output0, sampling_locations0, attention_weights0 = layer(src=output, pos=pos, reference_points=reference_points,
                                        spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                        key_padding_mask=key_padding_mask, layer_id=layer_id)
                        sampling_locations_list.append(sampling_locations0)
                        attention_weights_list.append(attention_weights0)
                        memory_list.append(output0)
                    elif layer_id == 1:
                        output1, sampling_locations1, attention_weights1 = layer(src=output0, pos=pos, reference_points=reference_points,
                                        spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                        key_padding_mask=key_padding_mask, layer_id=layer_id)
                        sampling_locations_list.append(sampling_locations1)
                        attention_weights_list.append(attention_weights1)
                        memory_list.append(output1)
                    elif layer_id == 2:
                        output2, sampling_locations2, attention_weights2 = layer(src=output1, pos=pos, reference_points=reference_points,
                                        spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                        key_padding_mask=key_padding_mask, layer_id=layer_id)
                        sampling_locations_list.append(sampling_locations2)
                        attention_weights_list.append(attention_weights2)
                        memory_list.append(output2)
                    elif layer_id == 3:
                        output2 = output2 + output0
                        output3, sampling_locations3, attention_weights3 = layer(src=output2, pos=pos, reference_points=reference_points,
                                        spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                        key_padding_mask=key_padding_mask, layer_id=layer_id)
                        sampling_locations_list.append(sampling_locations3)
                        attention_weights_list.append(attention_weights3)
                        memory_list.append(output3)
                    elif layer_id == 4:
                        output3 = output3 + output1
                        output4, sampling_locations4, attention_weights4 = layer(src=output3, pos=pos, reference_points=reference_points,
                                        spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                        key_padding_mask=key_padding_mask, layer_id=layer_id)
                        sampling_locations_list.append(sampling_locations4)
                        attention_weights_list.append(attention_weights4)
                        memory_list.append(output4)
                    elif layer_id == 5:
                        output4 = output4 + output2
                        output5, sampling_locations5, attention_weights5 = layer(src=output4, pos=pos, reference_points=reference_points,
                                        spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                        key_padding_mask=key_padding_mask, layer_id=layer_id)
                        output = output5
                        sampling_locations_list.append(sampling_locations5)
                        attention_weights_list.append(attention_weights5)
                        memory_list.append(output5)
                else:
                    # 正常的attention
        ...
        return output, intermediate_output, intermediate_ref, sampling_locations_list, attention_weights_list, memory_list
...
class Dynamic_conv1d(nn.Module):
    ...
    def forward(self, x):#将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的 2,13000,128
        softmax_attention = self.attention(x)
        batch_size, in_planes, height = x.size()
        x = x.reshape(1, -1, height)# 变化成一个维度进行组卷积
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if use_deformable_box_attn:
            ...
            self.self_attn = MSDeformAttn(d_model//2, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)

        # conv
        self.glu = nn.GLU()
        self.dynamic_conv_shallow = Dynamic_conv1d(64, 64, kernel_size=15, padding=7)
        self.dynamic_conv_deep = Dynamic_conv1d(64, 64, kernel_size=3, padding=1)
        self.linear_conv_shallow = nn.Linear(64, 128)
        self.linear_conv_deep = nn.Linear(64, 128)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None,**kwargs):
        # self attention
        # 方案一 :
        src_attn = src[..., :int(src.shape[2] / 2)]  # 前一半
        src_conv = src[..., int(src.shape[2] / 2):]  # 后一半

        src_attn_pos = pos[..., :int(pos.shape[2] / 2)]
        # src_conv_pos = pos[..., int(pos.shape[2] / 2):]

        # src_conv = self.with_pos_embed(src_conv, src_conv_pos)

        src_conv_shallow = src_conv[:, :level_start_index[1], :]
        src_conv_deep = src_conv[:, level_start_index[1]:, :]

        # 分层卷积
        src_conv_shallow = self.glu(src_conv_shallow)  # b,c,128->b,c,64
        src_conv_deep = self.glu(src_conv_deep)  # b,c,128->b,c,64
        src_conv_shallow_trans = torch.permute(src_conv_shallow, (0, 2, 1))  # b,c,64->b,64,c
        src_conv_deep_trans = torch.permute(src_conv_deep, (0, 2, 1))
        src_conv_shallow_trans = self.dynamic_conv_shallow(src_conv_shallow_trans)
        src_conv_deep_trans = self.dynamic_conv_deep(src_conv_deep_trans)
        src_conv_shallow = self.linear_conv_shallow(src_conv_shallow_trans.permute(0, 2, 1))  # b,64,c->b,c,128
        src_conv_deep = self.linear_conv_deep(src_conv_deep_trans.permute(0, 2, 1))  # b,64,c->b,c,128
        src_conv = torch.cat((src_conv_shallow, src_conv_deep), dim=1)  # bs, c, 128

        # 卷积部分
        # src_conv=self.glu(src_conv)# b,c,128->b,c,64
        # src_conv_trans = src_conv.reshape(1, src_conv.shape[0] * src_conv.shape[1], 64)
        # src_conv_trans=torch.permute(src_conv_trans,(0,2,1)) # b,c,64->b,64,c (1,64,b*c)
        # # src_conv_trans=src_conv_trans.reshape(1,64,src_conv.shape[0] * src_conv.shape[1])
        # src_conv_trans=self.dynamic_conv(src_conv_trans)# b,64,c  # 1,64,b*c -> b,64,c
        # src_conv_trans=src_conv_trans.reshape(src_conv.shape[0], 64, src_conv.shape[1])
        # src_conv=self.linear_conv(src_conv_trans.permute(0,2,1))# b,64,c->b,c,128
        # src_conv = self.glu(src_conv)  # b,c,128->b,c,64
        # src_conv_trans = torch.permute(src_conv, (0, 2, 1))  # b,c,64->b,64,c
        # src_conv_trans = self.dynamic_conv(src_conv_trans)  # b,64,c
        # src_conv = self.linear_conv(src_conv_trans.permute(0, 2, 1))  # b,64,c->b,c,128

        src2_attn, sampling_locations, attention_weights = self.self_attn(self.with_pos_embed(src_attn, src_attn_pos), reference_points, src_attn,
                                   spatial_shapes, level_start_index,
                                   key_padding_mask)
        src2 = torch.cat((src2_attn, src_conv), dim=2)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_ca(self,
        ...
        tgt2, sampling_locations_list, attention_weights_list = self.cross_attn(self.with_pos_embed(tgt, tgt_query_pos).transpose(0, 1),
                               tgt_reference_points.transpose(0, 1).contiguous(),
                               memory.transpose(0, 1), memory_spatial_shapes, memory_level_start_index,
